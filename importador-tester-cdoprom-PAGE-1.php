...
function crear_variaciones_del_producto($woocommerce, $product, $producto_padre_id, $cantidad_variaciones, $data_atributo_id, $nombre_atributo, $increment_value, $sku_padre, $page_api) {
    ...
    print("\n");
    status_message( 'Imprime array de product '. date("h:i:sa"));
    print_r($product);
    print("\n");

    // -------- Recorre para buscar todos los colores de las varaciones

    status_message( 'INICIA CICLO PARA...Buscar colores de variacion en variaciones a las: '. date("h:i:sa"));

    $color_idx = 0;

    foreach ($product['variants'] as $var_color) {

        if (isset($var_color['color'])) {
            $listado_colores_pp[] = $var_color['color']['name'];
        }
        
        if (isset($var_color['colors'])) {
            $listado_colores_pp[] = $var_color['colors'][1]['name'];
        }
    ...
    status_message( 'CONTEO VARIACIONES a las: '. date("h:i:sa") . ' ES : '. count($product['variants'])   );
    print("\n");
    ...
    status_message( 'INICIA CICLO PARA...Buscar llenar array con todas las '.$cantidad_variaciones.' variaciones a las: '. date("h:i:sa"));

    foreach ($product['variants'] as $variacion) {

        if (isset($variacion['color'])) {
            $nombre_term = $variacion['color']['name'];
        }
        
        if (isset($variacion['colors'])) {
            $nombre_term = $variacion['colors'][1]['name'];
        }
        ...
        $data = [
        'sku' => 'CDO-VAR-'.$variacion['id'],
        'manage_stock' => true,
        'variacion_stock_api' => $variacion['stock_available'],                
        'variacion_stock_anterior' => 0,                
        'id_variacion_api' => $variacion['id'],
        'color_variacion' => $nombre_term,
        'stock_quantity' => $variacion['stock_available'],                
        'regular_price' => number_format($precio_final, 2, '.', ''),
        'image' => [
            'src' => $variacion['picture']['original']
        ],
        'attributes' => [
            [
                'id' => $data_atributo_id,
    ...
    status_message( 'FINALIZA CICLO PARA...Buscar llenar array con todas las '.$cantidad_variaciones.' variaciones a las: '. date("h:i:sa"));
    print("\n");


    status_message( 'INICIA CREACION DE VARIACIONES a las '. date("h:i:sa"));

    // Se crean las variaciones via BATCH
    ...
    foreach ($batch_variaciones as $variacion) {
        ...
        crear_auditoria_1($producto_padre_id, $sku_padre, $variacion['id_variacion_api'], $status_auditoria, $cod_proveedor, $page_api, 0, $variacion_stock_api, $variacion_stock_actual);
    }
...
function actualizar_variaciones_del_producto($woocommerce, $product, $producto_padre_id, $cantidad_variaciones, $increment_value, $sku_padre, $page_api) {
    ...
    foreach ($product['variants'] as $index => $variacion) {
        ...
        if (!empty($variacion_id)) {
            ...
            // 1. Obtenemos la variación actual.
            $variacion_por_indice = $product['variants'][$index];

            // 2. Creamos un array temporal para forzar la clave "variants" en la impresión.
            // También incluimos el índice numérico como un sub-array para replicar el formato.
            ...
            crear_variaciones_del_producto($woocommerce, $output_array, $producto_padre_id, 1, $data_atributo_id, $nombre_atributo, $increment_value, $sku_padre, $page_api);
    ...
    if (!empty($batch_variaciones)) {
        ...
        foreach ($batch_variaciones as $variacion) {
            ...
            crear_auditoria_1($producto_padre_id, $sku_padre, $variacion['id_variacion_api'], $status_auditoria, $cod_proveedor, $page_api, $variacion['variacion_stock_anterior'], $variacion_stock_api, $variacion_stock_actual);
        }
...
function comparar_skus_local_vs_api_rest($woocommerce, $variaciones_del_producto_padre, $variaciones_desde_api, $status_auditoria_1, $cod_proveedor, $producto_padre_id, $sku_padre, $page_api, $variacion_stock_anterior, $variacion_stock_actual) {
    ...
    if (!empty($array_skus_incompatibles)) {
        ...
        foreach ($array_skus_incompatibles as $sku_a_insertar) {
            ...
            crear_auditoria_1($producto_padre_id, $sku_padre, $numero_extraido, $status_auditoria_1, $cod_proveedor, $page_api, $variacion_stock_anterior, 0, 0);
...
function crear_auditoria_1($producto_padre_id, $sku_padre, $id_variacion_api, $status_auditoria_1, $cod_proveedor, $page_api, $variacion_stock_anterior, $variacion_stock_api, $variacion_stock_actual) 
{
...
try {
    ...
    $sql = "INSERT INTO {$table_name} 
            (post_id, cod_proveedor, sku_padre, id_variacion, page_api, status_variacion_auditoria, variacion_stock_anterior, variacion_stock_api, variacion_stock_actual) 
            VALUES (:post_id, :cod_proveedor, :sku_padre, :id_variacion, :page_api, :status_variacion_auditoria, :variacion_stock_anterior, :variacion_stock_api, :variacion_stock_actual)";
