...
// Definimos la ruta del archivo a leer
const ARCHIVO_API_ORIGEN = 'lectura_api_cdo.js'; 
...
    
    ...
    print("\n");
    print("✔  HORA INICIO LECTURA ARCHIVO: " . date("h:i:sa") . " \n");
    
    // Leer el contenido del archivo
    $items_origin = @file_get_contents(ARCHIVO_API_ORIGEN);

    if ($items_origin === false) {
        exit('❗Error al leer el archivo de origen: ' . ARCHIVO_API_ORIGEN);
    }
    
    print("✔  HORA FIN LECTURA ARCHIVO: ". date("h:i:sa")." \n");
    
    $items_origin = json_decode($items_origin, true);
    
    // Asumimos que el JSON contiene la misma estructura que la respuesta de la API
    if (isset($items_origin['products'])) {
        $items_origin = $items_origin['products'];
    } else {
        // Manejo si la estructura es plana (solo productos) o si falla json_decode
        // Si el archivo solo contiene un array de productos sin el wrapper 'products', $items_origin ya estaría listo.
        // Si el decodificado falla, $items_origin puede ser null si el archivo está vacío.
        if (is_null($items_origin) && $items_origin !== false) {
            exit('❗Error: El contenido del archivo no es un JSON válido o está vacío.');
        }
        // Si no tiene la clave 'products', se asume que $items_origin es directamente el array de productos.
    }

    $cantidad_de_productos_api = count($items_origin);

    print("\n");
    status_message('Cantidad Productos Por SKU (desde archivo): '. $cantidad_de_productos_api);
    print("\n");
